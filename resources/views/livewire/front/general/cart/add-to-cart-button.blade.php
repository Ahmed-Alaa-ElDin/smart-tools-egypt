<button wire:click.stop="addToCart({{ $item_id }},'{{ $type }}')"
    title="@if ($add_buy == 'add') {{ __('front/homePage.Add to cart') }} @elseif($add_buy == 'pay') {{-- __('front/homePage.Go to payment') --}} {{ __('front/homePage.Opens soon') }} @elseif ($add_buy == 'buy'){{ __('front/homePage.Buy Again') }} @endif"
