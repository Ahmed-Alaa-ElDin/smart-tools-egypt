                            ...
                            @forelse ($products as $product)
                                ...
                                <tr>
                                            ...
                                            <div class="flex-shrink-0 h-10 w-10">
                                                @if ($product->thumbnail)
                                                    <img class="h-10 w-10 rounded-full"
                                                        src="{{ asset('storage/images/profiles/cropped200/' . $product) }}"
                                                        alt="{{ $product->name . 'profile image' }}">
                                                @else
                                                    <div
                                    ...
                                    <td class="px-6 py-2 text-center whitespace-nowrap">
                                        {{-- <div class="text-sm text-gray-900">{{ $product->email }}
                                        </div>
                                        <div class="text-sm text-gray-500">
                                            {{ $product->phones->where('default', 1)->first() ? $product->phones->where('default', 1)->first()->phone : '' }}
                                        </div> --}}
                                    </td>
                                    <td class="px-6 py-2 text-center whitespace-nowrap">
                                        {{-- {{ $product->balance . ' ' . __('admin/usersPages.LE') }} --}}
                                    </td>
                                    <td class="px-6 py-2 text-center whitespace-nowrap">
                                        {{-- {{ $product->visit_num }} --}}
                                    </td>
                                    <td class="px-6 py-2 text-center whitespace-nowrap text-sm text-gray-500">
                                        {{-- {{ $product->roles->first() ? $product->roles->first()->name : __('N/A') }} --}}
                                    </td>
                                    <td class="px-6 py-2 whitespace-nowrap text-center text-sm font-medium">
                                        ...
                                        @can("Edit User's Role")
                                            <a href="#" title="{{ __('admin/usersPages.Role') }}"
                                                wire:click.prevent="editRolesSelect({{ $product->id }})"
                                                class="m-0">
                                                <span
