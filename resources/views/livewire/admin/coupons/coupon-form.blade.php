<div class="grid grid-cols-12 gap-3 items-start">
    ...
    <div
        ...
        {{-- Code Start --}}
        <div class="col-span-12 md:col-span-6 grid grid-cols-3 gap-x-4 gap-y-2 items-center w-full">
            <label for="code"
        ...
        <div class="col-span-6 md:col-span-3 grid grid-cols-3 gap-x-4 gap-y-2 items-center w-full">
            ...
            <div class="col-span-3">
                ...
                <select wire:model="type" id="type"
                    class="rounded w-full cursor-pointer py-1 text-center border-red-300 focus:outline-red-600 focus:ring-red-300 focus:border-red-300 @error('type') border-red-900 border-2 @enderror">
        ...
        <div class="col-span-6 md:col-span-3 grid grid-cols-3 gap-x-4 gap-y-2 items-center w-full">
            ...
            <div class="col-span-3">
                <input id="value"
                    class="py-1 w-full rounded text-center border-red-300 focus:outline-red-600 focus:ring-red-300 focus:border-red-300 @error('value') border-red-900 border-2 @enderror"
                    type="text" wire:model.lazy="value" placeholder="{{ __('admin/offersPages.Enter Value') }}"
                    maxlength="100">
        ...
        <div
            class="col-span-12 md:col-span-6 lg:col-span-4 lg:col-start-3 grid grid-cols-3 gap-x-4 gap-y-2 items-center w-full">
            <label for="expire_at"
    ...
    <div
        ...
        @foreach ($items as $item_key => $item)
            ...
            <div class="col-span-12 bg-gray-200 w-full h-full p-3 grid grid-cols-12 gap-x-4 gap-y-4 rounded-xl items-center"
                ...
                <div class="col-span-12 bg-gray-700 p-3 rounded-xl flex flex-wrap justify-around items-center">
                ...
                <div class="col-span-12 grid grid-cols-12 gap-x-4 gap-y-4">
                    ...
                    @if ($items[$item_key]['item_type'] == 'brand')
                        ...
                        @if (!empty($items[$item_key]['products']))
                            <div class="col-span-12 grid grid-cols-3 items-center gap-3" wire:key="item-{{ $item_key }}-products">
                                <label
                ...
                <table class="w-100 table-bordered table-hover">
                    <tbody class="text-center">
                        @if (isset($oldSupercategories) && count($oldSupercategories))
                            <tr>
                                <th class="bg-primary text-white w-1/4">{{ __('admin/offersPages.Supercategories') }}</th>
                                    ...
                                    <div class="flex flex-wrap justify-center gap-3 items-center h-100">
                                        @foreach ($oldSupercategories as $supercategory_key => $supercategory)
                                            <label for="old-supercategory-{{ $supercategory_key }}"
                                                ...
                                                class="bg-red-200 px-3 py-1 min-w-max rounded-full text-black shadow cursor-pointer @if (in_array($supercategory['id'], $oldSupercategories_id)) bg-green-200 @endif select-none m-0">
                                                {{ $supercategory['name'][session('locale')] }}
                                                <input type="checkbox" wire:model="oldSupercategories_id"
                        ...
                        @if (isset($oldCategories) && count($oldCategories))
                            <tr>
                                <th class="bg-primary text-white w-1/4">{{ __('admin/offersPages.Categories') }}</th>
                                <td class="p-2 bg-white">
                                    <div class="flex flex-wrap justify-center gap-3 items-center h-100">
                                        @foreach ($oldCategories as $category_key => $category)
                                            <label for="old-category-{{ $category_key }}"
                                                wire:key="old-category-{{ $category_key }}"
                                                class="bg-red-200 px-3 py-1 min-w-max rounded-full text-black shadow cursor-pointer @if (in_array($category['id'], $oldCategories_id)) bg-green-200 @endif select-none m-0">
                                                {{ $category['name'][session('locale')] }}
                                                <input type="checkbox" wire:model="oldCategories_id"
                        ...
                        @if (isset($oldSubcategories) && count($oldSubcategories))
                            <tr>
                                <th class="bg-primary text-white w-1/4">{{ __('admin/offersPages.Subcategories') }}</th>
                                <td class="p-2 bg-white">
                                    <div class="flex flex-wrap justify-center gap-3 items-center h-100">
                                        @foreach ($oldSubcategories as $subcategory_key => $subcategory)
                                            <label for="old-subcategory-{{ $subcategory_key }}"
                                                ...
                                                class="bg-red-200 px-3 py-1 min-w-max rounded-full text-black shadow cursor-pointer @if (in_array($subcategory['id'], $oldSubcategories_id)) bg-green-200 @endif select-none m-0">
                                                {{ $subcategory['name'][session('locale')] }}
                                                <input type="checkbox" wire:model="oldSubcategories_id"
                                    ...
                                    <div class="flex flex-wrap justify-center gap-3 items-center h-100">
                                        @foreach ($oldBrands as $brand_key => $brand)
                                            <label for="old-brand-{{ $brand_key }}"
                                                ...
                                                class="bg-red-200 px-3 py-1 min-w-max rounded-full text-black shadow cursor-pointer @if (in_array($brand['id'], $oldBrands_id)) bg-green-200 @endif select-none m-0">
                                                {{ $brand['name'] }}
                                                <input type="checkbox" wire:model="oldBrands_id"
                                    ...
                                    <div class="flex flex-wrap justify-center gap-3 items-center h-100">
                                        @foreach ($oldProducts as $product_key => $product)
                                            <label for="old-product-{{ $product_key }}"
                                                ...
                                                class="bg-red-200 px-3 py-1 min-w-max rounded-full text-black shadow cursor-pointer @if (in_array($product['id'], $oldProducts_id)) bg-green-200 @endif select-none m-0">
                                                {{ $product['name'][session('locale')] }}
                                                <input type="checkbox" wire:model="oldProducts_id"
