<div class="top-slider container my-2 relative">
    <div class="grid grid-cols-12 gap-3 lg:gap-4">

        {{-- All Categories : Start --}}
        <div class="col-span-3 hidden lg:block z-10 h-64">
            <div class="flex justify-around items-center px-3 py-5 bg-red-100 rounded-t">
                <span class="font-bold text-sm">
                    {{ __('front/homePage.Main Categories') }}
                </span>
                <a href="#" class="font-bold text-xs">
                    {{ __('front/homePage.Show All') }}
                </a>

            </div>
            <ul class="bg-white rounded-b">
                <li class="group" data-id="1">
                    <div
                        class="relative w-full cursor-pointer group-hover:shadow p-1 group-hover:after:block after:hidden after:content-[''] after:w-7 after:h-7 after:rotate-45 ltr:after:border-t-8 ltr:after:border-r-8 rtl:after:border-b-8 rtl:after:border-l-8 after:border-white after:absolute ltr:after:-right-1 rtl:after:-left-1 after:top-2">
                        <a href="https://demo.activeitzone.com/ecommerce/category/women-clothing-fashion"
                            class="text-truncate text-reset py-2 px-3 block text-sm flex gap-3 items-center">
                            <span class="material-icons">
                                checkroom
                            </span>
                            <span class="cat-name">Women Clothing &amp; Fashion</span>
                        </a>
                    </div>

                    <div
                        class="group-hover:block hidden hover:block absolute w-75 bg-white h-full top-0 rtl:left-0 ltr:right-0 rounded shadow-lg p-4 loaded overflow-auto">
                        <div class="card-columns ">
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/watch-hyau2">Women
                                            Watch</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/women-dress-04vbn">Women
                                            Dress</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </li>

                <li class="group" data-id="1">
                    <div
                        class="relative w-full cursor-pointer group-hover:shadow p-1 group-hover:after:block after:hidden after:content-[''] after:w-7 after:h-7 after:rotate-45 ltr:after:border-t-8 ltr:after:border-r-8 rtl:after:border-b-8 rtl:after:border-l-8 after:border-white after:absolute ltr:after:-right-1 rtl:after:-left-1 after:top-2">
                        <a href="https://demo.activeitzone.com/ecommerce/category/women-clothing-fashion"
                            class="text-truncate text-reset py-2 px-3 block text-sm flex gap-3 items-center">
                            <span class="material-icons">
                                checkroom
                            </span>
                            <span class="cat-name">Women Clothing &amp; Fashion</span>
                        </a>
                    </div>

                    <div
                        class="group-hover:block hidden hover:block absolute w-75 bg-white h-full top-0 rtl:left-0 ltr:right-0 rounded shadow-lg p-4 loaded overflow-y-scroll">
                        <div class="card-columns ">
                            <div class="card shadow border-0 m-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hot-Categories">Hot
                                            Categories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Party-Dress">Party
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Beauty-&amp;-Health">Beauty
                                            &amp; Health</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Women-shoe">Women
                                            shoe</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Sleeping-Dress">Sleeping
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Casual-Dress">Casual
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hoodies-&amp;-Sweatshirts">Hoodies
                                            &amp; Sweatshirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jackets">Jackets</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/T-shirts">T-shirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Shirts">Shirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Socks">Socks</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wedding-&amp;-events">Wedding
                                            &amp; events</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wedding-Dresses">Wedding
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Evening-Dresses">Evening
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Prom-Dresses">Prom
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Bridesmaid-Dresses">Bridesmaid
                                            Dresses</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Bottom">Bottom</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Skirts">Skirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Leggings">Leggings</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jeans">Jeans</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Pant-&amp;-caprices">Pant
                                            &amp; caprices</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Shorts">Shorts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Formal-Pants">Formal
                                            Pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Casual-pants">Casual
                                            pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Cargo-pants">Cargo
                                            pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jeans">Jeans</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Tops-&amp;-sets">Tops
                                            &amp; sets</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Tank-tops">Tank
                                            tops</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Suit-&amp;-sets">Suit
                                            &amp; sets</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Accessories">Accessories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Eyewear-&amp;-accessories">Eyewear
                                            &amp; accessories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hats-&amp;-Caps">Hats
                                            &amp; Caps</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Gloves">Gloves</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Watch">Watch</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wallet">Wallet</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Belt">Belt</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hats-&amp;-Cap">Hats
                                            &amp; Cap</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/watch-hyau2">Women
                                            Watch</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/women-dress-04vbn">Women
                                            Dress</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </li>

                <li class="group" data-id="1">
                    <div
                        class="relative w-full cursor-pointer group-hover:shadow p-1 group-hover:after:block after:hidden after:content-[''] after:w-7 after:h-7 after:rotate-45 ltr:after:border-t-8 ltr:after:border-r-8 rtl:after:border-b-8 rtl:after:border-l-8 after:border-white after:absolute ltr:after:-right-1 rtl:after:-left-1 after:top-2">
                        <a href="https://demo.activeitzone.com/ecommerce/category/women-clothing-fashion"
                            class="text-truncate text-reset py-2 px-3 block text-sm flex gap-3 items-center">
                            <span class="material-icons">
                                checkroom
                            </span>
                            <span class="cat-name">Women Clothing &amp; Fashion</span>
                        </a>
                    </div>

                    <div
                        class="group-hover:block hidden hover:block absolute w-75 bg-white h-full top-0 rtl:left-0 ltr:right-0 rounded shadow-lg p-4 loaded overflow-y-scroll">
                        <div class="card-columns ">
                            <div class="card shadow-none border-0 m-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hot-Categories">Hot
                                            Categories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Party-Dress">Party
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Beauty-&amp;-Health">Beauty
                                            &amp; Health</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Women-shoe">Women
                                            shoe</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Sleeping-Dress">Sleeping
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Casual-Dress">Casual
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hoodies-&amp;-Sweatshirts">Hoodies
                                            &amp; Sweatshirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jackets">Jackets</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/T-shirts">T-shirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Shirts">Shirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Socks">Socks</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wedding-&amp;-events">Wedding
                                            &amp; events</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wedding-Dresses">Wedding
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Evening-Dresses">Evening
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Prom-Dresses">Prom
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Bridesmaid-Dresses">Bridesmaid
                                            Dresses</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Bottom">Bottom</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Skirts">Skirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Leggings">Leggings</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jeans">Jeans</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Pant-&amp;-caprices">Pant
                                            &amp; caprices</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Shorts">Shorts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Formal-Pants">Formal
                                            Pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Casual-pants">Casual
                                            pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Cargo-pants">Cargo
                                            pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jeans">Jeans</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Tops-&amp;-sets">Tops
                                            &amp; sets</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Tank-tops">Tank
                                            tops</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Suit-&amp;-sets">Suit
                                            &amp; sets</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Accessories">Accessories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Eyewear-&amp;-accessories">Eyewear
                                            &amp; accessories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hats-&amp;-Caps">Hats
                                            &amp; Caps</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Gloves">Gloves</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Watch">Watch</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wallet">Wallet</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Belt">Belt</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hats-&amp;-Cap">Hats
                                            &amp; Cap</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/watch-hyau2">Women
                                            Watch</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/women-dress-04vbn">Women
                                            Dress</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </li>

                <li class="group" data-id="1">
                    <div
                        class="relative w-full cursor-pointer group-hover:shadow p-1 group-hover:after:block after:hidden after:content-[''] after:w-7 after:h-7 after:rotate-45 ltr:after:border-t-8 ltr:after:border-r-8 rtl:after:border-b-8 rtl:after:border-l-8 after:border-white after:absolute ltr:after:-right-1 rtl:after:-left-1 after:top-2">
                        <a href="https://demo.activeitzone.com/ecommerce/category/women-clothing-fashion"
                            class="text-truncate text-reset py-2 px-3 block text-sm flex gap-3 items-center">
                            <span class="material-icons">
                                checkroom
                            </span>
                            <span class="cat-name">Women Clothing &amp; Fashion</span>
                        </a>
                    </div>

                    <div
                        class="group-hover:block hidden hover:block absolute w-75 bg-white h-full top-0 rtl:left-0 ltr:right-0 rounded shadow-lg p-4 loaded overflow-y-scroll">
                        <div class="card-columns ">
                            <div class="card shadow-none border-0 m-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hot-Categories">Hot
                                            Categories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Party-Dress">Party
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Beauty-&amp;-Health">Beauty
                                            &amp; Health</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Women-shoe">Women
                                            shoe</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Sleeping-Dress">Sleeping
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Casual-Dress">Casual
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hoodies-&amp;-Sweatshirts">Hoodies
                                            &amp; Sweatshirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jackets">Jackets</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/T-shirts">T-shirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Shirts">Shirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Socks">Socks</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wedding-&amp;-events">Wedding
                                            &amp; events</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wedding-Dresses">Wedding
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Evening-Dresses">Evening
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Prom-Dresses">Prom
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Bridesmaid-Dresses">Bridesmaid
                                            Dresses</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Bottom">Bottom</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Skirts">Skirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Leggings">Leggings</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jeans">Jeans</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Pant-&amp;-caprices">Pant
                                            &amp; caprices</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Shorts">Shorts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Formal-Pants">Formal
                                            Pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Casual-pants">Casual
                                            pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Cargo-pants">Cargo
                                            pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jeans">Jeans</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Tops-&amp;-sets">Tops
                                            &amp; sets</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Tank-tops">Tank
                                            tops</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Suit-&amp;-sets">Suit
                                            &amp; sets</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Accessories">Accessories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Eyewear-&amp;-accessories">Eyewear
                                            &amp; accessories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hats-&amp;-Caps">Hats
                                            &amp; Caps</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Gloves">Gloves</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Watch">Watch</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wallet">Wallet</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Belt">Belt</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hats-&amp;-Cap">Hats
                                            &amp; Cap</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/watch-hyau2">Women
                                            Watch</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/women-dress-04vbn">Women
                                            Dress</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </li>

                <li class="group" data-id="1">
                    <div
                        class="relative w-full cursor-pointer group-hover:shadow p-1 group-hover:after:block after:hidden after:content-[''] after:w-7 after:h-7 after:rotate-45 ltr:after:border-t-8 ltr:after:border-r-8 rtl:after:border-b-8 rtl:after:border-l-8 after:border-white after:absolute ltr:after:-right-1 rtl:after:-left-1 after:top-2">
                        <a href="https://demo.activeitzone.com/ecommerce/category/women-clothing-fashion"
                            class="text-truncate text-reset py-2 px-3 block text-sm flex gap-3 items-center">
                            <span class="material-icons">
                                checkroom
                            </span>
                            <span class="cat-name">Women Clothing &amp; Fashion</span>
                        </a>
                    </div>

                    <div
                        class="group-hover:block hidden hover:block absolute w-75 bg-white h-full top-0 rtl:left-0 ltr:right-0 rounded shadow-lg p-4 loaded overflow-y-scroll">
                        <div class="card-columns ">
                            <div class="card shadow-none border-0 m-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hot-Categories">Hot
                                            Categories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Party-Dress">Party
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Beauty-&amp;-Health">Beauty
                                            &amp; Health</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Women-shoe">Women
                                            shoe</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Sleeping-Dress">Sleeping
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Casual-Dress">Casual
                                            Dress</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hoodies-&amp;-Sweatshirts">Hoodies
                                            &amp; Sweatshirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jackets">Jackets</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/T-shirts">T-shirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Shirts">Shirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Socks">Socks</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wedding-&amp;-events">Wedding
                                            &amp; events</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wedding-Dresses">Wedding
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Evening-Dresses">Evening
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Prom-Dresses">Prom
                                            Dresses</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Bridesmaid-Dresses">Bridesmaid
                                            Dresses</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Bottom">Bottom</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Skirts">Skirts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Leggings">Leggings</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jeans">Jeans</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Pant-&amp;-caprices">Pant
                                            &amp; caprices</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Shorts">Shorts</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Formal-Pants">Formal
                                            Pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Casual-pants">Casual
                                            pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Cargo-pants">Cargo
                                            pants</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Jeans">Jeans</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Tops-&amp;-sets">Tops
                                            &amp; sets</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Tank-tops">Tank
                                            tops</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Suit-&amp;-sets">Suit
                                            &amp; sets</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Accessories">Accessories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Eyewear-&amp;-accessories">Eyewear
                                            &amp; accessories</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hats-&amp;-Caps">Hats
                                            &amp; Caps</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Gloves">Gloves</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Watch">Watch</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Wallet">Wallet</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Belt">Belt</a>
                                    </li>
                                    <li class="mb-2 text-sm">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/Hats-&amp;-Cap">Hats
                                            &amp; Cap</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/watch-hyau2">Women
                                            Watch</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="card shadow-none border-0 m-0">
                                <ul class="list-unstyled my-2 text-center">
                                    <li class="fw-600 border-y font-bold text-sm py-2 my-2">
                                        <a class="text-reset"
                                            href="https://demo.activeitzone.com/ecommerce/category/women-dress-04vbn">Women
                                            Dress</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </li>

                <li class="group flex justify-center items-center py-2">
                    <a href="#" class="btn bg-secondary text-white text-sm py-1 px-2 rounded m-0 font-bold">
                        {{ __('front/homePage.Show All') }}
                    </a>
                </li>
            </ul>
        </div>
        {{-- All Categories : End --}}

        <div class="col-span-12 lg:col-span-7 overflow-hidden rounded h-80">
            <div class="splide h-full w-full">
                <div class="splide__track">
                    <ul class="splide__list">
                        <li class="splide__slide"><img src="https://1.bp.blogspot.com/-ZzFhLpwkDkM/ViH7KIM7PkI/AAAAAAAAYGc/t7FazLK096U/s1600/Diwali%2BOnline%2BSales%2B%2521%2521%2BDiwali%2BDhamaka%2BSales%2BOffer%2B%2526%2BDiscount%2BBuy%2BOne%2BGet%2BOne%2BFree%2BMaalFreeKaa.jpg" alt="">
                        </li>
                        <li class="splide__slide"><img src="https://adoptostaging.blob.core.windows.net/media/informal-job-offer-letter-template-4oc2pH.jpg" alt="">
                        </li>
                        <li class="splide__slide"><img
                                src="{{ asset('assets/img/278183637_2564404720359953_2150832536107481886_n.jpg') }}"
                                alt=""></li>
                        <li class="splide__slide"><img
                                src="{{ asset('assets/img/278254292_2564404687026623_3366262232933159335_n.jpg') }}"
                                alt=""></li>
                        <li class="splide__slide"><img src="https://www.gla.ac.uk/media/Media_450788_smxx.jpg" alt="">
                        </li>
                        <li class="splide__slide"><img src="https://conceptboard.com/wp-content/uploads/5.png" alt="">
                        </li>
                    </ul>
                </div>
            </div>
        </div>

        {{-- Today's Deal : Start --}}
        <div class="col-span-12 lg:col-span-2 bg-white rounded h-72">
            <div class="flex justify-around items-center px-3 py-5 bg-red-100 rounded-t">
                <span class="text-sm font-bold">
                    {{ __("front/homePage.Today's Deal") }}
                </span>
                <span
                    class="text-xs font-bold rounded py-0.5 px-1 bg-red-600 text-white">{{ __('front/homePage.Hot') }}</span>
            </div>
            <div
                class="overflow-auto scrollbar scrollbar-thumb-secondary scrollbar-track-primary scrollbar-thin lg:h-64 p-2 bg-primary rounded-b">
                <div class="grid grid-cols-3 lg:grid-cols-1 gap-2">
                    <div class="col-span-1">
                        <a href="https://demo.activeitzone.com/ecommerce/product/Microsoft-Windows-10-pro-wVg2F"
                            class="block p-2 text-reset bg-white h-100 rounded">
                            <div class="items-center">
                                <div class="">
                                    <div class="img">
                                        <img class="img-fit lazyloaded w-24 m-auto"
                                            src="https://demo.activeitzone.com/ecommerce/public/uploads/all/VREgsy3guGKRJt4hXrUYzXIo6IWnJMwJ2N5Vv6cY.jpg"
                                            data-src="https://demo.activeitzone.com/ecommerce/public/uploads/all/VREgsy3guGKRJt4hXrUYzXIo6IWnJMwJ2N5Vv6cY.jpg"
                                            alt="Microsoft Windows 10 pro">
                                    </div>
                                </div>
                                <div class="">
                                    <div class="text-center">
                                        <span class="block text-primary fw-600">$35.000</span>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="col-span-1">
                        <a href="https://demo.activeitzone.com/ecommerce/product/sungait-ultra-lightweight-rectangular-polarized-sunglasses-uv400-protection-1szdb"
                            class="block p-2 text-reset bg-white h-100 rounded">
                            <div class="items-center">
                                <div class="">
                                    <div class="img">
                                        <img class="img-fit lazyloaded w-24 m-auto"
                                            src="https://demo.activeitzone.com/ecommerce/public/uploads/all/xIT7SLG4ciNvO2z4cU47GhtmS1RDiRBlnbzlYwSu.jpg"
                                            data-src="https://demo.activeitzone.com/ecommerce/public/uploads/all/xIT7SLG4ciNvO2z4cU47GhtmS1RDiRBlnbzlYwSu.jpg"
                                            alt="SUNGAIT Ultra Lightweight Rectangular Polarized Sunglasses UV400 Protection">
                                    </div>
                                </div>
                                <div class="">
                                    <div class="text-center">
                                        <span class="block text-primary fw-600">$14.000</span>
                                        <del class="block opacity-70">$25.000</del>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="col-span-1">
                        <a href="https://demo.activeitzone.com/ecommerce/product/aurora-13-inch-bonnie-teddy-bear-w5rkd"
                            class="block p-2 text-reset bg-white h-100 rounded">
                            <div class="items-center">
                                <div class="">
                                    <div class="img">
                                        <img class="img-fit lazyloaded w-24 m-auto"
                                            src="https://demo.activeitzone.com/ecommerce/public/uploads/all/eCLJnjhyfirOBFf864tcsbJO2o7kM2mpCa3meREZ.jpg"
                                            data-src="https://demo.activeitzone.com/ecommerce/public/uploads/all/eCLJnjhyfirOBFf864tcsbJO2o7kM2mpCa3meREZ.jpg"
                                            alt="Aurora 13-inch Bonnie Teddy Bear">
                                    </div>
                                </div>
                                <div class="">
                                    <div class="text-center">
                                        <span class="block text-primary fw-600">$50.000</span>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="col-span-1">
                        <a href="https://demo.activeitzone.com/ecommerce/product/apple---iphone-12-pro-max-5g-256gb-ohttv"
                            class="block p-2 text-reset bg-white h-100 rounded">
                            <div class="items-center">
                                <div class="">
                                    <div class="img">
                                        <img class="img-fit lazyloaded w-24 m-auto"
                                            src="https://demo.activeitzone.com/ecommerce/public/uploads/all/ct5MTUrbESEaVH71rdkenycBpltX2zwQdoK7b1o9.jpg"
                                            data-src="https://demo.activeitzone.com/ecommerce/public/uploads/all/ct5MTUrbESEaVH71rdkenycBpltX2zwQdoK7b1o9.jpg"
                                            alt="Apple - iPhone 12 Pro Max 5G 256GB">
                                    </div>
                                </div>
                                <div class="">
                                    <div class="text-center">
                                        <span class="block text-primary fw-600">$1,200.000</span>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="col-span-1">
                        <a href="https://demo.activeitzone.com/ecommerce/product/sportoli-maxi-dresses-for-women-solid-lightweight-long-racerback-sleeveless-wpocket-z3kgv"
                            class="block p-2 text-reset bg-white h-100 rounded">
                            <div class="items-center">
                                <div class="">
                                    <div class="img">
                                        <img class="img-fit lazyloaded w-24 m-auto"
                                            src="https://demo.activeitzone.com/ecommerce/public/uploads/all/xIT7SLG4ciNvO2z4cU47GhtmS1RDiRBlnbzlYwSu.jpg"
                                            data-src="https://demo.activeitzone.com/ecommerce/public/uploads/all/xIT7SLG4ciNvO2z4cU47GhtmS1RDiRBlnbzlYwSu.jpg"
                                            alt="SUNGAIT Ultra Lightweight Rectangular Polarized Sunglasses UV400 Protection">
                                    </div>
                                </div>
                                <div class="">
                                    <div class="text-center">
                                        <span class="block text-primary fw-600">$99.400</span>
                                        <del class="block opacity-70">$140.000</del>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                </div>
            </div>
        </div>
        {{-- Today's Deal : End --}}
    </div>
</div>
