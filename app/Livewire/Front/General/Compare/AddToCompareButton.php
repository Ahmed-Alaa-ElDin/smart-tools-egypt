...
class AddToCompareButton extends Component
{
    ...
    public function addToCompare($item_id, $type)
    {
        ...
        if (!$in_compare && Cart::instance('compare')->count() < 3) {
            ...
            $this->dispatch('swalDone', text: __('front/homePage.Product Has Been Added To The Comparison Successfully'),
                icon: 'success');
            ############ Emit Sweet Alert :: End ############
        } elseif (Cart::instance('compare')->count() >= 3) {
            $this->dispatch('swalDone', text: __('front/homePage.Comparison Limit Reached'),
                icon: 'error');
        } else {
            ############ Emit Sweet Alert :: Start ############
            $this->dispatch('swalDone', text: __('front/homePage.This product Is Already In The Comparison'),
                icon: 'error');
            ############ Emit Sweet Alert :: End ############
        }
